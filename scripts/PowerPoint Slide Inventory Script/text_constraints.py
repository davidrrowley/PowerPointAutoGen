...
def _validate_bullets(bullets: list[str], slide_num: int, field_name: str) -> None:
    if not isinstance(bullets, list):
        raise ValueError(f"Slide {slide_num}: field '{field_name}' must be a list of bullet strings.")
    ...
    for bullet in bullets:
        ...
        if len(bullet) > MAX_BULLET_CHARS:
            raise ValueError(
                f"Slide {slide_num}: a bullet in '{field_name}' exceeds {MAX_BULLET_CHARS} characters. "
                f"Bullet: '{bullet}'"
            )
...
def _validate_paragraph(text: str, slide_num: int, field_name: str) -> None:
    if not isinstance(text, str) or not text.strip():
        raise ValueError(f"Slide {slide_num}: field '{field_name}' must be a non-empty string.")

    if len(text) > MAX_PARAGRAPH_CHARS:
        raise ValueError(
            f"Slide {slide_num}: field '{field_name}' exceeds {MAX_PARAGRAPH_CHARS} characters."
        )
